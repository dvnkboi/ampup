...
$(window).ready(function(){
    ...
    function mainLoop() {
      ...
      setTimeout(function() { 
            if( songExpectedTime < currentState/2){
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if(( songExpectedTime < 1 || currentState < 1 ) && marker == 0){
                            console.log("song go BRRRRR");
                            songChanged++;
                            tmpOffset = metadataOffset;
                            if(metadataOffset>=8){
                                tmpOffset = metadataOffset - Math.sqrt(metadataOffset);
                                console.log("meta be like ",tmpOffset);
                            }
                            var audioSyncOffset = 0;
                            try{
                                audioSyncOffset = Math.abs(timer - Math.floor(audio.currentTime));
                            }
                            catch(e){
                                console.log("meta not good very bad");
                            }
                            if(correctedMeta == false){
                                setTimeout(function(){
                                    setMetadata(tmpOffset);
                                },audioSyncOffset * 1000 + 6000);                                
                            }
                        ...
                        songExpectedTime = parseInt(data);
                        currentState = parseInt(data);
                    }
                });     
            } 
        songExpectedTime-=0.5;
        resyncAudio();
        mainLoop();  
        ...
        if (prevType != e.type) { //  reduce double fire issues
            switch (e.type) {
                ...
                case "focus":
                    ...
                    success: function (data) {
                        try{
                            if( !songChanging && !correctedMeta && ((blured && songChanged>=1 && metaNoChange >= 1) || songExpectedTime < -8 || data.song[5].artist != globalData[5].artist || data.song[5].artist != $("#currentArtist").text() )){
                                forceMetadata();
                                metaChanged = true;
                                globalData = data.song.slice();
                                blured = false;
                                console.log("META WRONG",data.song[5].artist,globalData[5].artist);
                                metaNoChange = 0;
                                songChanged = 0;  
                                correctedMeta = true;
                                correctLimit=1;
                                ...
                                if((blured && songChanged>=1 && metaNoChange >= 1)){
                                    console.log("song go BRR but meta no go BRRR :c");
                                }
                                if(songExpectedTime< -8){
                                    console.log("timer too smol");
                                }
                                if(data.song[5].artist != globalData[5].artist){
                                    console.log("old playlist is dum new playlist better");
                                }
                                if(data.song[5].artist != $("#currentArtist").html()){
                                    console.log("artist is too old they died :c");
                                }
                                setTimeout(function(){
                                    correctedMeta = false;    
                                },2000);
...
function setMetadata(offset){
    ...
    success: function (data) {
        ...
        try{
            ...
            console.log("meta not good very bad :c");
        }
    }
    });
}
...
function metaAnimate(data){
    ...
    if(firstSetup != 0){
        ...
        fadeOut("#scrapedpic","left",100,function(){
            getImage(data.song[5].artist,"#scrapedpic");              
            fadeIn("#scrapedpic",1500);
        });
        ...
        fadeOut("#scrapedpic","left",0,function(){
            getImage(aurLess[5].artist,"#scrapedpic");
...
function setSessionMeta(data){
    if ('mediaSession' in navigator){
      navigator.mediaSession.metadata = new MediaMetadata({
        ...
        artwork:[{src:"https://ampupradio.com/assets/img/artists/" 
